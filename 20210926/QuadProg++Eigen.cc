...
// @tsumehashi. Compatible with Eigen.
...
void delete_constraint(Eigen::MatrixXd& R, Eigen::MatrixXd& J, std::vector<int>& A, Eigen::VectorXd& u, unsigned int n, int p, unsigned int& iq, int l);
...
void print_vector(const char* name, const VectorType& v, int n = -1);
...
#else
  const Eigen::LLT<Eigen::MatrixXd> lltOfG(G);
#endif
...
#else
  J = lltOfG.matrixL().toDenseMatrix().inverse().transpose();
  c2 = J.trace();
#endif
#ifdef TRACE_SOLVER
  print_matrix("J", J);
...
#else
  const int n = z.rows();
  const int blockSize = n-iq;
  z = J.block(0, iq, n, blockSize) * d.segment(iq, blockSize);
#endif
}
...
template<typename VectorType>
void print_vector(const char* name, const VectorType& v, int n)
{
  std::ostringstream s;
  std::string t;
  if (n == -1)
